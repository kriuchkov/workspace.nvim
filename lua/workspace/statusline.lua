...
function M.render()
  ...
  -- Filename + flags. Terminals fall back to the terminal title.
  local fname
  if bt == 'terminal' then
  ...
  -- Workspace name
  local ok_ws, ws = pcall(require, 'workspace.workspace')
  if ok_ws and ws._current then
    hl(t, 'CSWorkspace', '⬡ ' .. ws._current .. '  ')
  end

  -- Claude activity: spinner-count while jobs run. (No idle $-cost readout — it
  -- lingers after a command finishes and isn't useful there.)
  local ok_run, runner = pcall(require, 'workspace.claude.runner')
  if ok_run then
...
function M.setup()
  setup_highlights()
  vim.o.statusline = '%!v:lua.require("workspace.statusline").render()'
