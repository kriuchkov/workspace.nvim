...
local function groups_dir()
  return _session_dir_override or (vim.fn.stdpath('data') .. '/workspace_groups')
end

-- One-time migration from the pre-rename groups dir so tab groups survive.
do
  local old = vim.fn.stdpath('data') .. '/claudespace_groups'
  if vim.fn.isdirectory(old) == 1 and vim.fn.isdirectory(groups_dir()) == 0 then
    pcall(vim.fn.rename, old, groups_dir())
  end
end
...
function set_buf_safe(bufnr)
  ...
  require('workspace.shell').open(bufnr)
end
...
local function center_buf()
  local shell = require('workspace.shell')
...
function M.setup()
  ...
  vim.o.showtabline = 2
  vim.o.tabline = '%!v:lua.require("workspace.tabline").render()'
