...
-- A workspace is a root directory (the one holding .workspace/workspace.json)
...
local MANIFEST     = '.workspace/workspace.json'
local OLD_MANIFEST = '.claudespace/workspace.json'   -- pre-rename; still read for back-compat
...
local function find_root(start)
  ...
  while dir and dir ~= '' do
    if fn.filereadable(dir .. '/' .. MANIFEST) == 1
      or fn.filereadable(dir .. '/' .. OLD_MANIFEST) == 1 then return dir end
...
function M.load()
  ...
  if root then
    local mf = read_json(root .. '/' .. MANIFEST) or read_json(root .. '/' .. OLD_MANIFEST) or {}
...
function M.init_manifest()
  ...
  M.load()
  vim.notify(('workspace: wrote %s (%d repos)'):format(
...
function M.show()
  ...
  vim.keymap.set('n', '<CR>', function()
    ...
    close()
    pcall(function() require('workspace.filetree').focus_path(m.abspath) end)
  end, o)
  vim.keymap.set('n', 'g', function()
    local m = member_here(); if not m then return end
    close()
    pcall(function() require('workspace.git_ui').open(nil, m.abspath) end)
  end, o)
...
function M.show_info(path)
  ...
  local o = { buffer = buf, nowait = true, silent = true }
  vim.keymap.set('n', 'g', act(function() require('workspace.git_ui').open(nil, m.abspath) end), o)
  vim.keymap.set('n', '<CR>', act(function() require('workspace.filetree').focus_path(m.abspath) end), o)
  for _, k in ipairs { 'q', '<Esc>' } do vim.keymap.set('n', k, close, o) end
end
...
local function in_editor()
  pcall(function() require('workspace.claude.util').ensure_editor_win() end)
end
...
function M.setup()
  ...
  api.nvim_create_user_command('ClaudespaceWorkspaceInit', M.init_manifest,
    { desc = 'Generate .workspace/workspace.json from a repo scan' })
